const get = require("fsdb.js/database/get")
...
function retorno(db, req) {
    ...
    function fbPush(res) {
        return new Promise(async (resolve) => {
            const get = await fbGet();
            let array;
            if( get == undefined ) array = res
            if( get !== undefined ) array = get;
            if( get !== undefined ) array.push(res)
            const set = await fbSet(array);
            resolve(set)
        })
    }
